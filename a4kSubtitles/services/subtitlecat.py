...
from a4kSubtitles.lib import logger
from a4kSubtitles.lib import utils as a4k_utils
# from a4kSubtitles.lib import request as a4k_request # Consider using this
...
def build_search_requests(core, service_name, meta):
    ...
    if not query_title:
        core.logger.warning(f"[{service_name}] No title found in meta.")
        return []
    ...
    core.logger.info(f"[{service_name}] Search URL: {search_url}")
...
def parse_search_response(core, service_name, meta, response):
    core.logger.debug(f"[{service_name}] Parsing search response. Status: {response.status_code}")
    results = []
    ...
    try:
        ...
        core.logger.error(f"[{service_name}] Error creating BeautifulSoup object: {e}")
        return results
    ...
    try:
        ...
        core.logger.warning(f"[{service_name}] Could not get display_name from core.services: {e_disp}. Using service_name.")

    # --- PARSING SEARCH RESULTS PAGE ---
    # A.1: Main container for all results (specifically the tbody)
    results_table_body = soup.select_one('table.table.sub-table tbody')
    if not results_table_body:
        # Fallback: Sometimes the structure might be slightly different, or the class name subtly changes.
        # Let's try a more general approach if the specific one fails.
        results_table_body = soup.find('tbody') 
        if not results_table_body:
             core.logger.warning(f"[{service_name}] A.1: Main results table body ('table.sub-table tbody' or generic 'tbody') not found on {response.url}")
             return results
    
    # A.2: Individual result item selector (<tr> within the tbody)
    potential_movie_items_tr = results_table_body.find_all('tr')
    
    core.logger.info(f"[{service_name}] Found {len(potential_movie_items_tr)} potential movie rows on search page: {response.url}")

    for item_tr in potential_movie_items_tr:
        # A.3: Link to Detail Page (href)
        link_tag = item_tr.select_one('td:first-child > a') # Correct based on your HTML
        if not link_tag:
        ...
        # A.4: Display Title
        movie_title_on_page = link_tag.get_text(strip=True) # Correct based on your HTML
        ...
        try:
            # Consider using core.request.execute if it's robust for this
            ...
            core.logger.error(f"[{service_name}] Failed to fetch detail page {movie_page_full_url}: {e_detail}")
            continue

        # --- PARSING DETAIL PAGE ---
        # Extract info needed for download URLs from the detail page's own URL
        try:
            ...
            core.logger.error(f"[{service_name}] Could not parse ID and filename base from relative URL: {movie_page_relative_url}")
            continue


        # B.1: Main container for language list (iterating over 'div.sub-single')
        # B.2: Individual language entry is 'div.sub-single'
        language_entries = detail_soup.select('div.sub-single') # Correct based on your HTML
        ...
        for entry_div in language_entries:
            # B.3: Language code from <img> alt
            img_tag = entry_div.select_one('img.flag') # Correct based on your HTML
            if not img_tag:
                core.logger.debug(f"[{service_name}] No img.flag found in a language entry. Skipping entry.")
                continue
            sc_lang_code = img_tag.get('alt')
            if not sc_lang_code:
                core.logger.debug(f"[{service_name}] img.flag found but no alt attribute. Skipping entry.")
                continue

            # B.4: Full Language Name (second span)
            lang_name_span = entry_div.select_one('span:nth-of-type(2)') # Correct based on your HTML
            sc_lang_name_full = sc_lang_code # Fallback
            if lang_name_span:
                sc_lang_name_full = lang_name_span.get_text(strip=True)
            
            if not sc_lang_name_full: # If stripping made it empty or not found
                sc_lang_name_full = sc_lang_code # Fallback to the code

            try:
                kodi_target_lang_full = core.utils.get_language_name_from_code(sc_lang_code, core.kodi.xbmc.ENGLISH_NAME)
                # For lang_code in results, a4k seems to expect 2-letter ISO 639-1
                ...
                core.logger.warning(f"[{service_name}] Error converting lang code '{sc_lang_code}': {e_lang_conv}. Using raw values.")
            ...
            # Construct the direct .srt download link (for both "Download" and "Translate" cases)
            ...
            result_item = {
                ...
                'action_args': {
                    ...
                    'gzip': False, # Subtitlecat gives .srt directly
                    'service_name': service_name 
                }
            }
    ...
    core.logger.info(f"[{service_name}] Found {len(results)} subtitle results after parsing all pages.")
    return results

def build_download_request(core, service_name, args):
    download_url = args['url']
    filename_for_log = args.get('filename', download_url.split('/')[-1])
    core.logger.info(f"[{service_name}] Building download request for: {filename_for_log} from {download_url}")
